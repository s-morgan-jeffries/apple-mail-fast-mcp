...
**`from_account` parameter on the send-path tools (#155):** `send_email`, `send_email_with_attachments`, `reply_to_message`, and `forward_message` now accept an optional `from_account: str | None = None` parameter — pass an account name or UUID (matching the `account` convention elsewhere) to choose which configured Mail.app account sends the message. Resolves to the account's primary email address and sets it on the AppleScript `sender` property. Validation: raises `error_type: "account_not_found"` if no account matches. `from_account=None` (default) preserves Mail.app's default-sender behavior — fully backwards compatible. External contribution from @robertvitali.
