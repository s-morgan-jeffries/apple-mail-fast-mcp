...
| `from_account` | string | No | None | Mail.app account name or UUID. None = Mail's default. On a save-as-draft with exactly one enabled account, that account is adopted so the clean (no iOS quote bug) IMAP draft path can engage — it's Mail's default sender anyway, so the From is unchanged (#321). With more than one enabled account, set the `MAIL_DEFAULT_ACCOUNT` env var (name or UUID) so the same clean path can engage without passing `from_account` on every call — unlike the sole-account case, this **can** change the From header if it names an account other than Mail's own default sender; an unset or mismatched value falls back to the heuristic above. |
